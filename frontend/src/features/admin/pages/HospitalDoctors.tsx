...
export default function HospitalDoctors() {
  ...
  const loadAllData = async () => {
    try {
      const [docRes, msgRes, rateRes, apptRes] = await Promise.all([
        fetch("/api/admin/doctors"),
        fetch("/api/admin/doctors/messages"),
        fetch("/api/admin/doctors/ratings"),
        fetch("/api/admin/doctors/appointments"),
      ]);

      if (docRes.ok) setDoctors(await docRes.json());
      if (msgRes.ok) setMessages(await msgRes.json());
      if (rateRes.ok) setRatings(await rateRes.json());
  ...
  const handleSaveDoctor = async (e: React.FormEvent) => {
    ...
    try {
      if (editingDoctorId) {
        const res = await fetch(`/api/admin/doctors/${editingDoctorId}`, {
          method: "PUT",
  ...
  const toggleStatus = async (doc: Doctor) => {
    try {
      await fetch(`/api/admin/doctors/${doc.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...doc, active: !doc.active }),
      });
      loadAllData();
    } catch (error) {
  ...
  const handleDeleteMessage = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/doctors/messages/${id}`, { method: "DELETE" });
      if (res.ok) loadAllData();
    } catch (error) {
  ...
  const handleUpdateAppointmentStatus = async (id: string, status: "Pending" | "Confirmed" | "Cancelled") => {
    try {
      const res = await fetch(`/api/admin/doctors/appointments/${id}/status`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div className="max-w-2xl">
            ...
            <p className="mt-3 text-base text-slate-300">
              Manage doctor profiles, view patient secure messages, check doctor ratings, and handle appointment requests directly from your database.
            </p>
          </div>
