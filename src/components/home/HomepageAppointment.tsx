...
export default function AppointmentPage() {
  ...
  useEffect(() => {
    ...
    const activeServices = getServices().filter((s) => s.active);
    setServices(activeServices);

    // Check URL parameters or localStorage if a service was pre-selected from another page
    const params = new URLSearchParams(window.location.search);
    const serviceParam = params.get("service");
    
    if (serviceParam && activeServices.some((s) => s.id === serviceParam)) {
      setSelectedServiceId(serviceParam);
    } else if (activeServices.length > 0) {
      setSelectedServiceId(activeServices[0].id);
    }

    // Also support listening to storage or hash selection if passed dynamically
    const handleHashChange = () => {
      const hashService = window.location.hash.replace("#appointment-", "");
      if (activeServices.some((s) => s.id === hashService)) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      const existingRaw = localStorage.getItem(APPOINTMENTS_STORAGE_KEY);
      const existingAppointments = existingRaw ? JSON.parse(existingRaw) : [];

      // Save real appointment to localStorage for AdminDashboard integration
      localStorage.setItem(
        APPOINTMENTS_STORAGE_KEY,
        JSON.stringify([newAppointment, ...existingAppointments])
      );

      // Trigger custom storage event for sync across windows/tabs
      window.dispatchEvent(new Event("storage"));

      setStatusState({
        submitted: true,
        success: true,
        message: `Appointment successfully sent and logged for ${newAppointment.serviceName}. Our clinical coordinators will verify your slot shortly.`,
      });
    } catch (error) {
      setStatusState({
        submitted: true,
        success: false,
        message: "Booking Failed: Unable to save appointment record to browser storage. Please try again.",
      });
    }
  };
      ...
      <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mx-auto max-w-2xl text-center mb-12">
          ...
          <p className="mt-3 text-sm sm:text-base leading-relaxed text-slate-300">
            Schedule your consultation with Winston Medical Centre. Sent requests are instantly logged for our administration team.
          </p>
        </div>

        {statusState.submitted ? (
          /* STATUS FEEDBACK STATE (SUCCESS OR FAIL REASON) */
          ...
          <form onSubmit={handleSubmit} className="rounded-3xl border border-white/10 bg-gradient-to-b from-white/[0.08] to-white/[0.02] backdrop-blur-2xl p-6 sm:p-10 shadow-2xl space-y-6">
            ...
            <div className="pt-2">
              <button
                type="submit"
                className="w-full flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-cyan-500 via-teal-500 to-fuchsia-600 px-8 py-4 text-sm font-bold text-white shadow-xl shadow-cyan-500/20 transition-all hover:scale-[1.01] hover:shadow-fuchsia-500/30 active:scale-95"
              >
            ...
            <div className="flex items-center gap-2 text-xs text-slate-400 justify-center">
              ...
              <span>Real-time appointment requests sync instantly to the Winston administration portal.</span>
            </div>
          </form>
        )}
      </div>
