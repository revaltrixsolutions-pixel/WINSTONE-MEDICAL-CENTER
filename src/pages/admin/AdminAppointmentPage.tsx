      ...
      <header className="border-b border-white/10 bg-slate-900/60 backdrop-blur-xl sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] font-mono text-cyan-400 tracking-widest uppercase">Admin Portal</span>
          ...
          <div className="flex flex-wrap items-center gap-4 w-full lg:w-auto">
            ...
            <div className="flex items-center gap-2">
              ...
              {[
                ...
                { label: "Medical Services", value: "service" },
                { label: "Individual Doctors", value: "doctor" },
              ].map((src) => (
                <button
                  key={src.value}
                  type="button"
        ...
        <div className="rounded-3xl border border-white/10 bg-white/[0.02] backdrop-blur-xl overflow-hidden shadow-2xl">
          <div className="border-b border-white/10 px-6 py-4 flex items-center justify-between">
            <h3 className="font-bold text-white flex items-center gap-2">
              <Calendar size={18} className="text-cyan-400" /> All Appointment Requests (Services & Doctors)
            </h3>
          ...
          {filteredAppointments.length === 0 ? (
            <div className="p-16 text-center">
              ...
              <p className="mt-1 text-xs text-slate-400">Appointments booked from Medical Services or Individual Doctors will appear here.</p>
            </div>
          ) : (
            <div className="divide-y divide-white/10">
              {filteredAppointments.map((appt) => (
                <div key={appt.id || Math.random()} className="p-6 flex flex-col lg:flex-row lg:items-center justify-between gap-6 transition hover:bg-white/[0.04]">
                  
                  {/* Patient & Booking Details */}
                  <div className="space-y-2 min-w-0 flex-1">
                    <div className="flex flex-wrap items-center gap-3">
                      <h4 className="text-base font-bold text-white flex items-center gap-2">
                        <User size={16} className="text-cyan-400" /> {appt.fullName || "Unnamed Patient"}
                      </h4>

                      {/* Source Badge */}
                      <span className={`inline-flex items-center gap-1 font-mono text-[11px] px-2.5 py-0.5 rounded-full border ${
                        ...
                        {appt.sourceType === "doctor" ? `Doctor: ${appt.serviceName || "Specialist"}` : `Service: ${appt.serviceName || "General"}`}
                      </span>
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-xs text-slate-300">
                      {appt.email && (
                        <span className="flex items-center gap-1.5"><Mail size={14} className="text-slate-400" /> {appt.email}</span>
                      )}
                      {appt.phone && (
                        <span className="flex items-center gap-1.5"><Phone size={14} className="text-slate-400" /> {appt.phone}</span>
                      )}
                      {appt.date && (
                        <span className="flex items-center gap-1.5"><Calendar size={14} className="text-cyan-400" /> {appt.date}</span>
                      )}
                      {appt.time && (
                        <span className="flex items-center gap-1.5"><Clock size={14} className="text-cyan-400" /> {appt.time}</span>
                      )}
                    </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 shrink-0 self-start lg:self-center">
                    <select
                      ...
                      className="rounded-xl border border-white/20 bg-slate-900 px-3 py-2 text-xs font-bold text-white outline-none transition focus:border-cyan-400"
                    >
