...
const upload = multer({
    ...
    fileFilter: (_req, file, cb) => {
        ...
        cb(new Error("Unsupported file type. Use JPG, PNG, WEBP, GIF, MP4, WEBM or OGG."));
    },
});
/* =========================================================
   HELPERS
...
router.get("/:id", async (req, res) => {
    try {
        ...
        console.error("GET gallery item failed:", error);
        return res.status(500).json({
            success: false,
...
router.post("/", async (req, res) => {
    try {
        const { title, description, type, url, isActive, } = req.body;
        ...
        const galleryType = normalizeType(type);
        const galleryUrl = galleryType === "embed"
...
router.post("/upload", upload.single("file"), async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({
                success: false,
                message: "No gallery file was uploaded.",
            });
        }
        const { title, description, type, isActive, } = req.body;
        const detectedType = type === "embed"
            ? "embed"
        ...
        return res.status(201).json({
            success: true,
            message: "Gallery file uploaded and saved successfully.",
            item,
        });
    }
    catch (error) {
...
router.patch("/:id", async (req, res) => {
    try {
        ...
        if (isActive !== undefined) {
            data.isActive =
                normalizeBoolean(isActive, existing.isActive);
        }
